 ...
 *
 * Handles market analysis results from the polymarket service.
...
export class PolymarketStream {
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`[polymarketStream] Reconnecting in ${delay}ms (attempt ${this.retryCount}/${this.maxRetries})`);
  ...
  // Send transcript to polymarket for analysis (if needed for client-initiated)
...
let instance: PolymarketStream | null = null;

export function getPolymarketStream(options?: PolymarketStreamOptions): PolymarketStream {
  if (!instance) {
    instance = new PolymarketStream(options);
  }
  return instance;
}

export function resetPolymarketStream(): void {
  if (instance) {
    instance.disconnect();
    instance = null;
  }
